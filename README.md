# Local Claude Code with Qwen3-Coder-Next-NVFP4

Run Claude Code against a local [Qwen3-Coder-Next-NVFP4](https://huggingface.co/GadflyII/Qwen3-Coder-Next-NVFP4) model (80B MoE, ~45GB NVFP4) using vLLM and LiteLLM.
...
- NVIDIA GPU with CUDA 12.8+ drivers (tested on RTX PRO 6000 Blackwell 96GB)
...
The first run will download the model (~45GB) and install Python dependencies.
...
1. **vLLM** serves the Qwen3-Coder-Next-NVFP4 model with an OpenAI-compatible API, with tool calling enabled via `--tool-call-parser qwen3_coder` and FP8 KV cache for memory efficiency
